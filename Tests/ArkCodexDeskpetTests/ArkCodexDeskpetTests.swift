...
final class ArkCodexDeskpetTests: XCTestCase {
    ...
    func testTaskSummaryUsesExplicitRequestAndTruncatesLongText() {
        let message = "# Files mentioned\nignored context\n## My request:\n现在气泡框中显示的状态信息太模糊简略了"
        XCTAssertEqual(compactTaskSummary(message, limit: 10), "现在气泡框中显示的状…")
        XCTAssertEqual(compactTaskSummary("调整图标", limit: 10), "调整图标")
    }

    func testWorkBuddyStatusDistinguishesLifecycleStates() {
        XCTAssertEqual(workBuddyStatus(isRunning: false, isActive: false).text, "WorkBuddy 未运行")
        XCTAssertEqual(workBuddyStatus(isRunning: true, isActive: false).text, "WorkBuddy 已打开")
        XCTAssertEqual(workBuddyStatus(isRunning: true, isActive: true).text, "WorkBuddy 使用中")
    }
